...
def REarth(X, Y, M=1):
    ...
    return sse, [coeffs], np.array([no_of_terms]), interactions, rearth;

# Predict using the MARS model
def predict_mars(X_new, rearth):
    # Convert X_new into an R matrix with the appropriate dimensions
    if X_new.ndim == 1:
        # If X_new is a 1D array (single sample), reshape it into a 2D array with one sample and the correct number of features
        X_new = X_new.reshape(1, -1)

    rX_new = r.matrix(X_new, nrow=X_new.shape[0], ncol=X_new.shape[1], byrow=False)

    # Predict using the MARS model
    pred_r = MARS.predict_earth(rearth, rX_new)

    # Convert the prediction result to a numpy array and reshape it to match the input shape
    return np.array(pred_r).flatten()  # Flatten to ensure it's a 1D array
